-- ============================================================================
-- Integration Cockpit — DEMO SEED  (stands in for NiFi; run AFTER Phase 0 DDL)
-- Postgres / Neon. Populates every table the dashboard reads, including the
-- edge cases each acceptance criterion tests. No NiFi required.
-- ============================================================================

-- 0. clean (optional, for re-runs)
...
-- ============================================================================
-- 1. BULK VOLUME  (~300k rows; bump to 2-5M to load-test perf)
--    mostly healthy, small fail/reject/dup, prod + uat, EDI ~74% / API ~26%
-- ============================================================================
...
INSERT INTO endpoint_health (channel, endpoint, partner, environment, status, last_ok_at, cert_expires_at) VALUES
 ('sftp','crocs-edi','Crocs','prod','down',      now()-interval '22 min', now()+5),   -- down + cert 5d
 ('api','rxo-track','RXO','prod','degraded',     now()-interval '2 min',  now()+3),   -- 401s + token 3d
 ('as2','maersk-as2','Maersk','prod','up',       now()-interval '20 sec', now()+11),
 ('sftp','target-sftp','Target','prod','up',     now()-interval '15 sec', now()+120),
 ('sftp','crocs-edi-uat','Crocs','uat','up',     now()-interval '30 sec', now()+90);
...
-- DONE. The dashboard now renders fully on this seed. Replace with NiFi later (see NiFi must-do guide).
