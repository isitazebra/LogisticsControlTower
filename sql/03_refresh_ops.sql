...
BEGIN
  ...
  UPDATE ops_monitor_heartbeat SET last_run_at = now() - interval '18 min', expected_interval_sec = 300
    WHERE monitor_name = 'van-liveness';
  UPDATE ops_monitor_heartbeat SET last_run_at = now(), expected_interval_sec = 86400
  ...
  UPDATE ops_endpoint_health SET last_ok_at=now()-interval '22 min', cert_expires_at=(now()+interval '5 day')::date  WHERE endpoint='crocs-edi';
  UPDATE ops_endpoint_health SET last_ok_at=now()-interval '2 min',  cert_expires_at=(now()+interval '3 day')::date  WHERE endpoint='rxo-track';
  UPDATE ops_endpoint_health SET last_ok_at=now()-interval '20 sec', cert_expires_at=(now()+interval '11 day')::date WHERE endpoint='maersk-as2';
  UPDATE ops_endpoint_health SET last_ok_at=now()-interval '15 sec', cert_expires_at=(now()+interval '120 day')::date WHERE endpoint='target-sftp';
  UPDATE ops_endpoint_health SET last_ok_at=now()-interval '30 sec', cert_expires_at=(now()+interval '90 day')::date  WHERE endpoint='crocs-edi-uat';

  -- Pipeline health: walgreens-tl HUNG (state+queue+rate, time-independent); others consuming.
  UPDATE ops_pipeline_health SET last_consumed_at=now()-interval '40 min' WHERE pipeline='walgreens-tl';
  UPDATE ops_pipeline_health SET last_consumed_at=now()                   WHERE pipeline IN ('air-main','ocean-main');

  -- Expected feeds: Werner 204 + Kroger 856 MISSING (past); Maersk 214 healthy (far future).
  UPDATE ops_expected_feeds SET expected_next_at=now()-interval '35 min', last_seen_at=now()-interval '95 min'
    WHERE partner='Werner' AND doc_type='204';
  UPDATE ops_expected_feeds SET expected_next_at=now()-interval '2 hour', last_seen_at=now()-interval '26 hour'
